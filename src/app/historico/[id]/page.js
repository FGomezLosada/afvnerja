...
export default function DetalleTemporada() {
  ...
  const [premiosTemporada, setPremiosTemporada] = useState(null)
  ...
  return (
    <div style={{ maxWidth: '1000px', margin: '0 auto', padding: '32px 24px' }}>
      ...
      {premiosTemporada && premiosTemporada.premios?.length > 0 && (
        <div style={{
          backgroundColor: 'var(--blanco)',
          ...
          <h2 style={{ color: '#B07800', fontSize: '16px', fontWeight: '700', marginBottom: '16px', textAlign: 'center' }}>
            🏆 Premios Final de Temporada {temporada?.nombre}
          </h2>
          <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
            {premiosTemporada.premios.map((p, i) => (
              <div key={i} style={{ display: 'flex', alignItems: 'flex-start', gap: '12px', padding: '12px', backgroundColor: 'rgba(176,120,0,0.08)', borderRadius: '8px', border: '1px solid rgba(176,120,0,0.2)' }}>
                <div>
                  <div style={{ fontSize: '13px', fontWeight: '600', color: '#B07800', marginBottom: '2px' }}>{p.categoria}</div>
                  <div style={{ fontSize: '15px', fontWeight: '700', color: 'var(--azul-marino)' }}>{p.ganador}</div>
                  {p.descripcion && <div style={{ fontSize: '13px', color: '#666', marginTop: '2px' }}>🎁 {p.descripcion}</div>}
                </div>
              </div>
            ))}
          </div>
